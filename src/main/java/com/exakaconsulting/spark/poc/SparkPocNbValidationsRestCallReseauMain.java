...
public class SparkPocNbValidationsRestCallReseauMain {
	...
	public static void main(String[] args) throws Exception {
		...
		csvValidationBilStation = csvValidationBilStation.groupBy(STATION_COLUMN).sum("NB_VALD")
				.withColumnRenamed("sum(NB_VALD)", NBRE_VALIDATION).orderBy(STATION_COLUMN);

		List<String> listStation = new ArrayList<>();
		// collectAsList has to be avoided because it retrieves all the data to
		// the driver.
		...
		Dataset<Row> datasetResultCallRest = sparkSession.read()
				.format("org.apache.dsext.spark.datasource.rest.RestDataSource").options(options).load();
		...
		Dataset<Row> csvJointure = csvValidationBilStation
				.join(datasetResultCallRest, JavaConversions.asScalaBuffer(listColumns), LEFT_OUTER)
				.orderBy(STATION_COLUMN).withColumn("listCorrespondance",
						functions.to_json(functions.struct(functions.col("listCorrespondance"))));

		if (directory.contains("hdfs://")) {
			csvJointure.rdd().saveAsTextFile(directory + "/outputfile.csv");
			csvJointure.coalesce(1).write().mode(OVERRIDE_CSTE).options(getDataOutputParamCsv())
					.csv(directory + "/output.csv");

		} else {
			csvJointure.write().mode(OVERRIDE_CSTE).options(getDataOutputParamCsv()).csv(directory + "/output.csv");
			csvJointure.coalesce(1).write().mode(OVERRIDE_CSTE).options(getDataOutputParamCsv())
					.csv(directory + "/outputfinal.csv");
